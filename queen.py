...
class Queen(Piece):
    ...
    def visible_tiles(self, board):
        pair = visible_lines(board, self.get_tile(), self.get_colour(),[ROOK_LINE, BISHOP_LINE])
        tiles = pair[1]
    ...
    def valid_tiles(self, board, checking_pieces, pinner, enemy_visible):
        self.set_valid_moves(valid_lines(board, checking_pieces, pinner, self.get_tile(), self.get_colour(), [ROOK_LINE, BISHOP_LINE]))